...
func exampleRecipeInfo(dataType string, data string) rbtypes.RecipeInfoChunk {
	return rbtypes.RecipeInfoChunk{
		DataType: dataType,
		Data:     data,
	}
}

func ingredientList(listData rbtypes.ExampleData, recipeID string) []rbtypes.Ingredient {
	list := make([]rbtypes.Ingredient, len(listData))
...
func instructionList(listData rbtypes.ExampleData, recipeID string) []rbtypes.Instruction {
	list := make([]rbtypes.Instruction, len(listData))
	for i, data := range listData {
		list[i] = exampleRecipeInfo(data[1], data[0])
	}
	return list
...
func exampleRecipe(name string, i int) rbtypes.Recipe {
	user := UserJDWblip
...
func exampleDemoRecipe(name string, i int) rbtypes.RecipeDemo {
	user := UserJDWblip
	recipeID := strconv.Itoa(i)
	ingredients := ingredientList(exampleIngredientData, recipeID)
	instructions := instructionList(exampleInstructionData, recipeID)
	meta := rbtypes.RecipeDemoMetaInput{
		Name:   name,
		Author: user.Username,
		Banner: "",
	}

	return rbtypes.RecipeDemo{
		ID:           recipeID,
		MetaInfo:     meta,
		Ingredients:  ingredients,
		Instructions: instructions,
...
var exampleIngredientData rbtypes.ExampleData = rbtypes.ExampleData{
	{"Fried Chicken", "title"},
...
var exampleInstructionData rbtypes.ExampleData = rbtypes.ExampleData{
...
var UserJDWblip = rbtypes.UserBlip{
	Image:       "",
	DisplayName: "John Whiteside",
	Username:    "jwhiteside11",
}

var UserJDWlogin = rbtypes.UserLoginInfo{
	Email:    "jwhiteside11@yahoo.com",
	Username: "jwhiteside11",
	Password: "password123",
}

var RegisteredUserMap = map[string]rbtypes.UserLoginInfo{
	"0": UserJDWlogin,
}

var VerifiedUserLoginMap = map[string]rbtypes.UserLoginInfo{}

var VerifiedUserMap = map[string]rbtypes.User{
	"0": {
		ID:            "0",
		Email:         "jwhiteside11@yahoo.com",
		Blip:          UserJDWblip,
		Occupation:    "Home cook",
		Allergies:     make([]rbtypes.Setting, 0),
		FavoriteFoods: make([]rbtypes.Setting, 0),
	},
}

var RecipeMap = map[string]rbtypes.Recipe{
...
var RecipeDemoMap = map[string]rbtypes.RecipeDemo{
...
var RecipeMetaInfoMap = map[string]rbtypes.RecipeMetaInfo{
